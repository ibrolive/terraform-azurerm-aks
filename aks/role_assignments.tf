...
resource "azurerm_role_assignment" "application_gateway_existing_vnet_network_contributor" {
  count = var.create_role_assignments_for_application_gateway && local.use_brown_field_gw_for_ingress && length(azurerm_kubernetes_cluster.main.ingress_application_gateway) > 0 && length(azurerm_kubernetes_cluster.main.ingress_application_gateway[0].ingress_application_gateway_identity) > 0 ? 1 : 0
...
resource "azurerm_role_assignment" "application_gateway_byo_vnet_network_contributor" {
  count = var.create_role_assignments_for_application_gateway && local.use_green_field_gw_for_ingress && length(azurerm_kubernetes_cluster.main.ingress_application_gateway) > 0 && length(azurerm_kubernetes_cluster.main.ingress_application_gateway[0].ingress_application_gateway_identity) > 0 ? 1 : 0
...
resource "azurerm_role_assignment" "application_gateway_resource_group_reader" {
  count = var.create_role_assignments_for_application_gateway && local.ingress_application_gateway_enabled && length(azurerm_kubernetes_cluster.main.ingress_application_gateway) > 0 && length(azurerm_kubernetes_cluster.main.ingress_application_gateway[0].ingress_application_gateway_identity) > 0 ? 1 : 0
